angular.module('starter.controllers')
    .controller('ClientViewDeliveryCtrl',[
        '$scope', '$stateParams', 'ClientOrder', 'UserData', '$ionicLoading',
        '$ionicPopup', '$pusher', '$window','$map','uiGmapGoogleMapApi',
        function($scope, $stateParams, ClientOrder, UserData, $ionicLoading,$ionicPopup,$pusher,$window,$map,uiGmapGoogleMapApi){
            ...
            uiGmapGoogleMapApi.then(function (maps) {
                ...
                $ionicPopup.alert({
                    title: 'Adivertência',
                    template: 'mapa não pode ser carregado!'
                });
            });
            ...
            function createMarkerClient(address) {
                ...
                geocoder.geocode({
                    ...
                    if(status == google.maps.GeocoderStatus.OK){
                        ...
                        $ionicPopup.alert({
                            title: 'Adivertência',
                            template: 'Não foi possivel encontrar o endereço cadastrado! :,('
                        });
                    }
                });
            }
            ...
            function watchPositionDeliveryman(channel){
                var pusher = $pusher($window.client), channel = pusher.subscribe(channel);
                channel.bind('CodeDelivery\\Events\\GetLocationDeliveryman', function(data) {
                    ...
                    for(var key in $scope.markers){
                        if($scope.markers[key].id == 'entrgador'){
            ...
            function createBounds() {
                ...
                angular.forEach($scope.markers,function (values) {
                    latlng = new google.maps.LatLng(Number(values.coords.latitude),Number(values.coords.longitude));
    ...
    .controller('CdvControlDescentralize',['$scope','$map',function ($scope,$map) {
        $scope.map = $map;
        $scope.fit = function () {
    ...
    .controller('CdvControlReload',['$scope','$window','$timeout',function ($scope ,$window ,$timeout) {
